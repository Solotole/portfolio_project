...
@app_views.route('/users/<user_id>/read', methods=['POST'],
...
def mark_as_read(user_id):
    """ Mark a book as read
        Args:
            user_id: user id
        Return:
            return a list of deserializes book id
...
@app_views.route('/users/<user_id>/read', methods=['GET'],
                 strict_slashes=False)
def get_read_books(user_id):
    """ Get books a user has read
        Args:
            user_id: user id
        Return:
            returns list of all marked books by a user
    """
